...
describe('Test suite', function () {
    ...
    describe('createSessionId && saveToStorage && getUser && deleteSession && createAuthToken && updateTableId', function() {
        it('should pass the test', async function () {
            const hashedPassword = await bcrypt.hash(password, 10);
            const queryString = 'INSERT INTO users (name, password, email, image) VALUES (?, ?, ?, ?)'
            const [result] = await mysql.pool.execute(queryString, [tokenUser.name, hashedPassword, tokenUser.email, tokenUser.image]) as any
            ...
            const newSession = await _session.updateTableId(tableId)
